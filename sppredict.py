...
df = pd.read_csv('estimators_sp_raw.csv', sep=',',header=0)
...
df.loc[:,'PPG'] = np.divide(np.sum([df.loc[:,'Outs'],df.loc[:,'W']*5,df.loc[:,'SV']*5,df.loc[:,'SO'],df.loc[:,'H']*-1,df.loc[:,'BB']*-1,df.loc[:,'IBB']*-1,df.loc[:,'ER']*-2],axis=0),df.loc[:,'GS'])
...
corr1 = np.corrcoef([bbp_estimate,ppg])[0,1]
corr2 = np.corrcoef([fip_estimate,ppg])[0,1]
corr3 = np.corrcoef([fipficiency_estimate,ppg])[0,1]

print(corr1*corr1,corr2*corr2,corr3*corr3)

#Run with test data
df = pd.read_csv('estimators_sp_test.csv', sep=',',header=0)
